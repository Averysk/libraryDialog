...
public class BubbleLayout extends FrameLayout {
    ...
    private void initAttr(TypedArray a) {
        ...
        mLookPosition = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleLookPosition, 0);
        mLookWidth = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleLookWidth, DialogUtil.dip2px(getContext(), 17F));
        mLookLength = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleLookLength, DialogUtil.dip2px(getContext(), 17F));
        mShadowRadius = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleShadowRadius, DialogUtil.dip2px(getContext(), 3.3F));
        mShadowX = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleShadowX, DialogUtil.dip2px(getContext(), 1F));
        mShadowY = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleShadowY, DialogUtil.dip2px(getContext(), 1F));
        mBubbleRadius = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubbleRadius, DialogUtil.dip2px(getContext(), 7F));
        mBubblePadding = a.getDimensionPixelOffset(R.styleable.BubbleLayout_bubblePadding2, DialogUtil.dip2px(getContext(), 1));
        mShadowColor = a.getColor(R.styleable.BubbleLayout_bubbleShadowColor2, Color.GRAY);
...
//    private int mLeft, mTop, mRight, mBottom;
    ...
    /**
     * 新: 手动设置气泡箭头自动位置(根据点击的View)
     */
    public void setLookPositionAuto(View view) {
        ...
        switch (mLook) {
            case TOP:
            case BOTTOM: {
                int x = clickedViewLocation[0] + mClickedRect.width() / 2 - this.getMeasuredWidth() / 2;
                if (x <= 0) {
                    this.mLookPosition = clickedViewLocation[0] + mClickedRect.width() / 2 - this.getLookWidth() / 2;
                } else if (x + this.getMeasuredWidth() > DialogUtil.getScreenSize(getContext())[0]) {
                    this.mLookPosition = clickedViewLocation[0] - (DialogUtil.getScreenSize(getContext())[0] - this.getMeasuredWidth()) + mClickedRect.width() / 2 - this.getLookWidth() / 2;
                } else {
                    this.mLookPosition = clickedViewLocation[0] - x + mClickedRect.width() / 2 - this.getLookWidth() / 2;
                }
            }
                break;
            case LEFT:
            case RIGHT: {
                int y = clickedViewLocation[1] + mClickedRect.height() / 2 - this.getMeasuredHeight() / 2 - DialogUtil.getStatusHeight(getContext());
                if (y <= 0) {
                    this.mLookPosition = clickedViewLocation[1] + mClickedRect.height() / 2 - this.getLookWidth() / 2 - DialogUtil.getStatusHeight(getContext());
                } else if (y + this.getMeasuredHeight() > DialogUtil.getScreenSize(getContext())[1]) {
                    this.mLookPosition = clickedViewLocation[1] - (DialogUtil.getScreenSize(getContext())[1] - this.getMeasuredHeight()) + mClickedRect.height() / 2 - this.getLookWidth() / 2;
                } else {
                    this.mLookPosition = clickedViewLocation[1] - y + mClickedRect.height() / 2 - this.getLookWidth() / 2 - DialogUtil.getStatusHeight(getContext());
                }
            }
                break;
        }
